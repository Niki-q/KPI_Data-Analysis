...
summary(data)
```
Фунція побудови box plots:
```{r}

box_plot <- function(data, aes_y, title_y, title_size, text_size) {
  ggplot(data, aes_y) +
    geom_boxplot() +
    labs(y = title_y) +
...
popularity:
```{r}
box_plot(data, aes(y = popularity), "Popularity of tracks", 25,20)
```
valence:
```{R}
box_plot(data, aes(y = valence), "Valence of tracks", 40,40)

```
duration_ms: (треба переробити!)
```{R}
box_plot(data, aes(y = duration_ms), "Duration of tracks", 40,40)

```
energy:
```{R}
box_plot(data, aes(y = energy), "The energy of the tracks", 40,40)

```
tempo:
```{r}
box_plot(data, aes(y = tempo), "Tempo tracks", 40,40)

```
speechiness:
```{r}
box_plot(data, aes(y = speechiness), "The speechiness of the tracks", 40,40)
```
